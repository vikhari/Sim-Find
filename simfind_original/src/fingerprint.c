...
int fingerprint_compare(FINGERPRINT *fingerprint1, FINGERPRINT *fingerprint2) {
    ...
    //In case of file-comparison we need the bigger value
    ...
    for(i=0;i<=smaller_fingerprint->amount_of_BF;i++) {
    	if(bf->amount_of_blocks < MINBLOCKS) 	//there is no sense in comparing Bloom filters having less than MINBLOCKS Blocks
    		break;
...
int bloom_max_score(BLOOMFILTER *bf, FINGERPRINT *fingerprint) {
    ...
    unsigned int numofbitsInCommon = 0;
    ...
    while (tmp_bf != NULL) {
    	...
    	//Filters with MINBLOCKS or less elements are critical
 ...
 //  	    C = 0.35*(e_max - e_min)+e_min;
//   	    C = MAX(40,C);

       	//compute bits in common
        numofbitsInCommon = bloom_common_bits(tmp_bf->array, bf->array);

        //last against last (often produces FPs)
        //if(bf->next == NULL || tmp_bf->next == NULL)
        	//C = C*1.2;

        //if they are high enough we have a threshold
        tmp_score = 0;
        if(numofbitsInCommon > C && (e_max - C) >= 1) {
