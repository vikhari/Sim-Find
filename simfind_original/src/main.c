...
int main(int argc, char **argv){
	...
	if(mode->print || optind==1) {
	  ...
	  for (int j = optind; j < argc; j++)
		  addPathToBloomfilter(fpl, argv[j]);
	...
	if(mode->gen_compare) {
	  ...
	  if(mode->path_list_compare){
		  ...
		  for (int j = optind; j < argc; j++)
		  			  addPathToBloomfilter(fpl2, argv[j]);
	  ...
	  if(mode->compare) {
		  FINGERPRINT_LIST *fpl1 = init_empty_fingerprintList();
	  	  ...
	  	  addPathToBloomfilter(fpl1, argv[optind]);
	  	  addPathToBloomfilter(fpl2, argv[optind+1]);
 ...
 */
void addPathToBloomfilter(FINGERPRINT_LIST *fpl, char *filename){
	DIR *dir;
	struct dirent *ent;
		  	...
		  	while ((ent = readdir (dir)) != NULL) {
		  		...
		  		//when we found a dir and recursive mode is on, go deeper
		  			...
		  			if(strcmp(ent->d_name, ".") == 0 || strcmp(ent->d_name, "..") == 0)
		  			    continue;
		  			addPathToBloomfilter(fpl, ent->d_name);
		  		}
		  	}
		  	chdir(cur_dir);
		  	closedir (dir);
